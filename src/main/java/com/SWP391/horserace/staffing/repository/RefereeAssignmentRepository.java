...
public interface RefereeAssignmentRepository extends JpaRepository<RefereeAssignment, UUID> {
    ...
    List<RefereeAssignment> findByRace_RaceIdAndStatusNot(UUID raceId, String status);

    /** Check if a referee is already assigned to a race (non-REVOKED). */
    boolean existsByRace_RaceIdAndReferee_UserIdAndStatusNot(UUID raceId, UUID refereeUserId, String status);

    /** Count active (non-REVOKED) assignments for a specific referee. */
    long countByReferee_UserIdAndStatusNot(UUID refereeUserId, String status);

    /** Count distinct races that have at least one active assignment. */
    @Query("SELECT COUNT(DISTINCT ra.race.raceId) FROM RefereeAssignment ra WHERE ra.status <> 'REVOKED'")
    long countDistinctAssignedRaces();

    /** Find active assignment by id (not REVOKED). */
    Optional<RefereeAssignment> findByRefAssignmentIdAndStatusNot(UUID refAssignmentId, String status);
    ...
    @Query("SELECT ra.referee.userId, COUNT(ra) FROM RefereeAssignment ra " +
           "WHERE ra.status <> 'REVOKED' GROUP BY ra.referee.userId")
